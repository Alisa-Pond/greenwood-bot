...
def default_player(user_id):
    return {
        ...
        "plant_archive": [],
        # -------------------------------------------------
        # СТАРЕ ПОЛЕ QUESTS (для сумісності)
        # -------------------------------------------------
...
def update_player(user_id, player_data):
    try:
        ...
        # Не оновлюємо ідентифікатори та застарілі поля
...
def get_player(user_id):
    ...
    try:
        ...
        if response.data:
            ...
            # АКТИВНІ КВЕСТИ
            for column in ("scrolls", "rituals", "plants", "expeditions"):
                if player.get(column) is None:
                    player[column] = []
                    changed = True

            # АРХІВИ
            for column in ("scroll_archive", "ritual_archive", "plant_archive"):
                if player.get(column) is None:
                    player[column] = []
                    changed = True
            ...
            if player.get("statistics") is None:
                ...
                for key, default_value in DEFAULT_STATISTICS.items():
                    if key not in statistics:
                        statistics[key] = copy.deepcopy(default_value)
...
def save_scroll(user_id, scroll):
    try:
        ...
        new_title = str(scroll.get("title", "")).strip()
        normalized_new_title = new_title.casefold()

        for existing_scroll in scrolls:
            existing_title = str(existing_scroll.get("title", "")).strip()
            if existing_title.casefold() == normalized_new_title:
                print(f"⚠️ Дубль сувою для {user_id}: {new_title}")
                return {
                    "success": False,
                    "duplicate": True,
                    "count": len(scrolls),
        ...
        success = update_player(user_id, {"scrolls": scrolls})
        ...
        print(f"📜 Сувій '{new_title}' збережено для {user_id}.")
        return {
            "success": True,
            "duplicate": False,
            "count": len(scrolls),
...
def save_ritual(user_id, ritual):
    try:
        ...
        success = update_player(user_id, {"rituals": rituals})
